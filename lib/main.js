...
const assumeRole = (aws, roleArn) => new Promise((resolve,reject) => {
	const timestamp = (new Date()).getTime();
	const params = {
		RoleArn: roleArn,
		RoleSessionName: `node-lambda-deploy-${timestamp}`
	};
...
Lambda.prototype.deploy = async function( program ) {
	...
	if (
		(typeof $config.AWS_SESSION_TOKEN === "object") &&
		Object.prototype.hasOwnProperty.call( $config.AWS_SESSION_TOKEN , 'Ref') &&
		(typeof $config.AWS_SESSION_TOKEN.Ref === "string") &&
		($config.AWS_SESSION_TOKEN.Ref.indexOf('env.') === 0)
	)
		$config.AWS_SESSION_TOKEN = process.env[ $config.AWS_SESSION_TOKEN.Ref.split('env.')[1] ]

	if (
		(typeof $config.AWS_STS_ROLE_ARN === "object") &&
		Object.prototype.hasOwnProperty.call( $config.AWS_STS_ROLE_ARN , 'Ref') &&
		(typeof $config.AWS_STS_ROLE_ARN.Ref === "string") &&
		($config.AWS_STS_ROLE_ARN.Ref.indexOf('env.') === 0)
	)
		$config.AWS_STS_ROLE_ARN = process.env[ $config.AWS_STS_ROLE_ARN.Ref.split('env.')[1] ]
	...
	if ($config.AWS_STS_ROLE_ARN) {
		const data = await assumeRole(aws, $config.AWS_STS_ROLE_ARN)
		
		$config.AWS_KEY = data.Credentials.AccessKeyId
		$config.AWS_SECRET = data.Credentials.SecretAccessKey
		$config.AWS_SESSION_TOKEN = data.Credentials.SessionToken
	}
					...
					setTimeout(() => {
						...
						lambdaV2.updateFunctionCode(paramsV2, function( err, data ) {
							...
							if ($config.ProvisionedConcurrentExecutions) {
								lambdaV2.listProvisionedConcurrencyConfigs({
									FunctionName: $config.FunctionName,
								}, async function(err, concurrencyConfigs) {
									for (const config of concurrencyConfigs.ProvisionedConcurrencyConfigs) {
										await new Promise((resolve, reject) => {
											const version = /arn:aws:lambda:[\w-]+:\d+:function:.*:(\d+)/i.exec(config.FunctionArn)[1]
											if (version)
												lambdaV2.deleteProvisionedConcurrencyConfig({
													FunctionName: $config.FunctionName,
													Qualifier: version
												}, function(err, provisionedData) {
													if (err) reject(err)
													resolve(provisionedData)
												})
										})
									}
									if (err) console.log("WARNING: Failed to delete provisioned concurrency config:", err )
									
									lambdaV2.putProvisionedConcurrencyConfig({
										FunctionName: $config.FunctionName,
										Qualifier: data.Version,
										ProvisionedConcurrentExecutions: $config.ProvisionedConcurrentExecutions
									}, function (err) {
										if (err) console.log("WARNING: Failed to put provisioned concurrency config:", err )
